...
namespace NPuzzle
{
    internal class AStar
    {
        ...
        public List<Node> Astar_seaech(char way)
        {
            ...
            pq.Enqueue(startNode, startNode.AstareScore(way)); //  // O(N^2)
            ...
            while (pq.Count > 0)
            {
                ...
                if (node.solved(way)) // O(N^2)
                {
                    return node.SolutionPath(); // O(L)
                }
                //List<Move> list= new List<Move>();
                //list=node.Actions();// O(N^2)

                foreach (Move move in node.Actions())  
                {
                    ...
                    {
                        pq.Enqueue(child, child.AstareScore(way)); // O(N^2)
                        seen.Add(step); // O(1)
                    }
